...
class ProductPresenter extends ModelPresenter
{
    ...
    public function price()
    {
        if (! $this->model->sale_price->value()) {
            return new HtmlString(sprintf('%s<span itemprop="price">%s</span>', $this->model->price->symbol(), $this->model->price->asDecimal());
        }

        return new HtmlString(sprintf(
      '<del>%s</del> <ins>%s</ins>',
      $this->model->price,
      sprintf('%s<span itemprop="price">%s</span>', $this->model->sale_price->symbol(), $this->model->sale_price->asDecimal()
      ));
    }
